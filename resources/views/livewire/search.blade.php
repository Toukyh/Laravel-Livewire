<div
    ...
    <input type="text"
    class="w-full bg-white rounded-xl border dark:bg-gray-600 dark:bg-opacity-30  border-gray-300 focus:border-indigo-500 focus:ring-2 text-base outline-none text-gray-700 dark:text-gray-100 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" @click.prevent="open = true" wire:model="query"
    ...
    @if (strlen($query) >= 2)
        ...
        @if (count($jobs) > 0)
            @foreach ($jobs as $index => $job)
                <a href="#" class="{{ ($index === $selectedIndex) ? 'text-indigo-400' : '' }} my-2">{{ $job['title'] }}</a>
            @endforeach
        @else
